...
def sample_examples(docs,max_window_size,n_windows):
    ...
    for i,doc in enumerate(docs):
        ...
        windows.append(get_windows(doc, np.random.randint(1,max_window_size+1)))
    ...
    all_negs =  list(np.random.choice(list(vocab.values()), size = n_negs*len(windows),p = neg_distr)) ### fill the gap (sample n_negs*len(windows) negatives according to some probability distribution ###
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    ### fill the gaps ###
    if pos:
        partials_pos = np.array([-1*factors*Wt[target,] for factors in factors_pos])
    else :
        partials_pos = np.zeros((0,d))
        
    partials_negs = np.array([factors*Wt[target,] for factors in factors_negs])
    
    term_pos = np.array([-1*Wc[posi,]*factors for factors,posi in zip(factors_pos,pos)])
    term_negs = np.array([Wc[neg]*factors for factors,neg in zip(factors_negs,negs)])
...
path_read = '/Users/adriano/Desktop/ALTEGRAD/lab2/data/'
...
if not train:
    ...
    print(my_cos_similarity('film','movie'))
    ...
    fig, ax = plt.subplots()
    ax.scatter(my_tsne_fit[:n_plot,0],my_tsne_fit[:n_plot,1],s=3) ### fill the gap ###
    for x,y,token in zip(my_tsne_fit[:,0],my_tsne_fit[:,1],mft[0:n_plot]): ### fill the gap ###

...
def cnn_branch(n_filters,k_size,d_rate,my_input):
    conv = Conv1D(n_filters, k_size, activation ='relu')(my_input)
    max_pool = GlobalMaxPooling1D()(conv)
    return Dropout(d_rate)(max_pool)
...
doc_wv = Embedding(input_dim = len(word_to_index)+2 , output_dim = d)(doc_ints) ### fill the gap (add an Embedding layer) https://keras.io/layers/embeddings/ ###
...
saliency_output = model.output[0][0] ### fill the gap (get the probability distribution over classes from the model) ###

...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        hs = self.rnn(embedding)
        #print('source',hs.size())
...
class seq2seqAtt(nn.Module):
    ...
    def forward(self,target_h,source_hs):
        ...
        #print('concat',concat_output.size())
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        #print(h.size())
        #print(torch.cat((source_context,h),2).size())
        embedding = self.embedding(input)
        output, h = self.rnn(embedding,h)
        #output = self.softmax(self.out(output[0]))
        concat = self.ff_concat(torch.cat((source_context,h),2))
        tilde_h =  torch.tanh(concat)#.unsqueeze(0)
...
class seq2seqModel(nn.Module):
    ...
    def forward(self,input,max_size,is_prod, for_plot = False):
        ...
        # fill the gap #
        # use the encoder
        source_hs = self.encoder.forward(input)[0]
        ...
        while True: 
            ...
            _, target_input = prediction.max(2) 
    ...
    def show_alignment(self, source_nl, idx):
        source_ints = self.sourceNl_to_ints(source_nl)
        logits, scores = self.forward(source_ints, self.max_size,True,True)  
        ...
        fig.colorbar(cax)
        source = source_nl.split()
        ax.set_xticklabels([''] + [x for x in source])
